...
void AMyThirdPersonChar::MoveRight(float Value) 
{
	...
	{
		const FRotator YawRotaion(0, Controller->GetControlRotation().Yaw, 0);

		const FVector Direction = UKismetMathLibrary::GetRightVector(YawRotaion);
...
void AMyThirdPersonChar::MoveForward(float Value)
{
	if (Controller != nullptr && Value != 0.0f)
	{
		...
		const FVector Direction = UKismetMathLibrary::GetForwardVector(YawRotaion);
